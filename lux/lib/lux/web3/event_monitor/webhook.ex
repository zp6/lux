defmodule Lux.Web3.EventMonitor.Webhook do
  ...
  Provides HTTP POST delivery with configurable retry logic (exponential backoff),
  HMAC-SHA256 signature headers for verification, and a dead letter queue for
  failed deliveries.
  ...
  def unregister(webhook_id), do: GenServer.call(__MODULE__, {:unregister, webhook_id})

  @spec list_webhooks() :: {:ok, [webhook()]}
  def list_webhooks, do: GenServer.call(__MODULE__, :list_webhooks)

  @spec deliver(map()) :: :ok
  def deliver(event), do: GenServer.cast(__MODULE__, {:deliver, event})

  @spec dead_letter_queue() :: {:ok, [delivery()]}
  def dead_letter_queue, do: GenServer.call(__MODULE__, :dead_letter_queue)

  @spec retry_dead_letter() :: :ok
  def retry_dead_letter, do: GenServer.cast(__MODULE__, :retry_dead_letter)

  @spec purge_dead_letter() :: :ok
  def purge_dead_letter, do: GenServer.call(__MODULE__, :purge_dead_letter)
  ...
  def init(_opts) do
    ...
    {:ok, %{webhooks: webhooks, dead_letter: [], dead_letter_max: get_config(:dead_letter_max, 1000)}}
  end
  ...
  def handle_call({:register, attrs}, _from, state) do
    ...
    if is_nil(url) or url == "" do
      ...
      webhook = %{
        ...
        delivery_count: 0, last_delivery_at: nil
      }
  ...
  def handle_call({:unregister, webhook_id}, _from, state) do
    case Map.pop(state.webhooks, webhook_id) do
      ...
      {_wh, remaining} -> {:reply, :ok, %{state | webhooks: remaining}}
    end
  end
  ...
  def handle_cast({:deliver, event}, state) do
    ...
    matching = state.webhooks |> Map.values() |> Enum.filter(fn wh ->
    ...
    new_state = Enum.reduce(matching, state, fn wh, s -> deliver_webhook(wh, event, s) end)
    {:noreply, new_state}
  end
  ...
  def handle_cast(:retry_dead_letter, state) do
    ...
    {remaining} = Enum.reduce(state.dead_letter, {[]}, fn delivery, {rem} ->
  ...
  defp deliver_webhook(webhook, event, state) do
    ...
    delivery = %{
      id: generate_id(), webhook_id: webhook.id, url: webhook.url,
      payload: event, signature: signature, status: :pending,
      attempts: 0, last_attempt_at: nil, last_error: nil, created_at: DateTime.utc_now()
    }
  ...
  defp do_http_post_with_payload(url, payload, signature, extra_headers) do
    ...
    custom = (extra_headers || %{}) |> Enum.map(fn {k, v} -> {to_string(k), to_string(v)} end)
    timeout = get_config(:timeout_ms, 10_000)

    case Req.post(url, json: payload, headers: headers ++ custom, receive_timeout: timeout) do
  ...
  defp do_http_post(delivery) do
    ...
    case Req.post(delivery.url, json: payload, headers: headers, receive_timeout: timeout) do
      {:ok, %{status: code}} when code >= 200 and code < 300 -> :ok
      {:ok, %{status: code}} -> {:error, {:http_error, code}}
      {:error, reason} -> {:error, reason}
    end
  ...
  defp persist_webhooks(webhooks) do
    ...
    try do
      data = webhooks |> Map.values() |> Enum.map(fn wh ->
        %{
          "id" => wh.id, "url" => wh.url, "events" => wh.events,
          "contract_address" => wh.contract_address, "headers" => wh.headers,
          "created_at" => DateTime.to_iso8601(wh.created_at), "delivery_count" => wh.delivery_count
        }
      end) |> Jason.encode!()
  ...
  defp load_persisted_webhooks do
    ...
    if File.exists?(path) do
      try do
        data = File.read!(path)
        Jason.decode!(data) |> Enum.map(fn wh ->
          ...
          {wh["id"], %{
            ...
            created_at: created_at, delivery_count: wh["delivery_count"] || 0,
  ...
  defp webhook_persistence_path, do: Application.app_dir(:lux, "priv/web3/event_monitor_webhooks.json")
end
