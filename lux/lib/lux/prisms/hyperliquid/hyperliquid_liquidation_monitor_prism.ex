defmodule Lux.Prisms.Hyperliquid.HyperliquidLiquidationMonitorPrism do
  ...
  for positions approaching liquidation prices.
  ...
  ## Examples
      ...
      ...>   address: "0x..."
      ...> }, %{})
      {:ok, %{risk_level: "low", positions: [...], liquidation_risks: [...]}}
  """

  use Lux.Prism,
    name: "Hyperliquid Liquidation Monitor",
    description: "Monitors liquidation risk and provides alerts for Hyperliquid positions",
    input_schema: %{
      type: :object,
      properties: %{
        address: %{
        ...
        warning_threshold: %{
          type: :number,
          description: "Distance to liquidation as percentage to trigger warning (default: 20%)",
          default: 20.0
        },
        critical_threshold: %{
          type: :number,
          description: "Distance to liquidation as percentage to trigger critical alert (default: 10%)",
          default: 10.0
        }
      },
      required: ["address"]
  ...
  defp fetch_user_state(api_url, address) do
    ...
    case Req.request(Req.new(url: api_url, method: :post, json: body)) do
  ...
  defp calculate_risk(position, prices, warning_threshold, critical_threshold) do
    ...
    distance_pct =
      if liq_price > 0 and current_price > 0 do
