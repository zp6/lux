defmodule Lux.Prisms.Hyperliquid.HyperliquidPnLTrackerPrism do
  ...
  use Lux.Prism,
    ...
    input_schema: %{
      ...
      properties: %{
        address: %{
          type: :string,
          description: "Ethereum wallet address to track PnL for",
          pattern: "^0x[a-fA-F0-9]{40}$"
        },
        include_closed: %{
          type: :boolean,
          description: "Include closed position PnL history",
          default: false
        }
      },
      required: ["address"]
  ...
  alias Lux.Integrations.Telegram.Client, as: _TelegramClient
  require Logger
  ...
  defp parse_positions(%{"assetPositions" => positions}) do
    parsed = Enum.map(positions, fn %{"position" => pos} = asset ->
      ...
      entry_px = parse_float(pos["entryPx"])
