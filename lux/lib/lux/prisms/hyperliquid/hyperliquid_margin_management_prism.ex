defmodule Lux.Prisms.Hyperliquid.HyperliquidMarginManagementPrism do
  ...
  Supports adding and removing margin from isolated positions,
  and transferring between cross-margin and isolated margin modes.
  ...
  use Lux.Prism,
    ...
    input_schema: %{
      ...
      properties: %{
        ...
        amount: %{
          type: :number,
          description: "Amount of margin to add or remove (in USD)"
        },
        action: %{
          type: :string,
          description: "Margin action: 'add' or 'remove'",
          enum: ["add", "remove"]
        },
        vault_address: %{
          type: :string,
  ...
  defp validate_action(action), do: {:error, "Invalid action: #{action}. Must be 'add' or 'remove'"}
end
