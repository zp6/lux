defmodule Lux.Prisms.Telegram.Webhook.SetWebhookPrism do
  ...
  with support for custom certificates, allowed updates, and secret tokens.
  ...
  use Lux.Prism,
    ...
    input_schema: %{
      ...
      properties: %{
        ...
        ip_address: %{type: :string, description: "Fixed IP address for DNS resolution"},
        max_connections: %{type: :integer, description: "Max simultaneous connections (1-100, default 40)", minimum: 1, maximum: 100},
        allowed_updates: %{type: :array, items: %{type: :string}, description: "Update types to receive"},
        drop_pending_updates: %{type: :boolean, description: "Drop all pending updates", default: false},
    ...
    output_schema: %{
      type: :object,
      properties: %{set: %{type: :boolean}, url: %{type: :string}, description: %{type: :string}},
      required: ["set", "url"]
    }
  ...
  def handler(params, _ctx) do
    ...
    body = params |> Map.take([:url, :ip_address, :max_connections, :allowed_updates, :drop_pending_updates, :secret_token]) |> Enum.reject(fn {_k, v} -> is_nil(v) end) |> Map.new()

    case Client.request(:post, "/setWebhook", %{json: body}) do
      {:ok, %{"ok" => true, "result" => true, "description" => desc}} ->
        {:ok, %{set: true, url: url, description: desc}}
      {:ok, %{"ok" => true, "result" => true}} ->
        {:ok, %{set: true, url: url, description: "Webhook was set"}}
      {:ok, %{"ok" => false, "description" => desc}} ->
        {:error, "Failed to set webhook: #{desc}"}
      {:error, error} ->
