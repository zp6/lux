defmodule Lux.Prisms.Telegram.Webhook.DeleteWebhookPrism do
  ...
  Switches back to getUpdates polling mode.
  ...
  use Lux.Prism,
    ...
    description: "Removes webhook and switches to getUpdates polling",
    input_schema: %{
      type: :object,
      properties: %{drop_pending_updates: %{type: :boolean, description: "Drop all pending updates", default: false}}
    },
    output_schema: %{
      type: :object,
      properties: %{deleted: %{type: :boolean}, description: %{type: :string}},
      required: ["deleted"]
    }
  ...
  def handler(params, _ctx) do
    body = Map.take(params, [:drop_pending_updates])
    case Client.request(:post, "/deleteWebhook", %{json: body}) do
